...
ax.set_ylim(0,250)
ax.grid(True)
ax.errorbar(d1[:,0], d1[:,3], yerr=d1[:,4]/np.sqrt(d1[:,0]), fmt='r^--', linewidth=0.8,markersize=7, markeredgecolor='red', fillstyle='none', capsize=3, label = "w/ Triton on GKE-4gpu, dynamic batching Off")
ax.errorbar(d4[:,0], d4[:,3], yerr=d4[:,4]/np.sqrt(d4[:,0]), fmt='bv--', linewidth=0.8,markersize=7, markeredgecolor='blue', fillstyle='none', capsize=3, label = "w/ Triton on GKE-4gpu, dynamic batching On")
...
ax2.set_ylim(0,350)
ax2.grid(True)
ax2.errorbar(d1[:,0], d1[:,1], yerr=d1[:,2]/np.sqrt(d1[:,0]), fmt='r^--', linewidth=0.8,markersize=7, markeredgecolor='red', fillstyle='none', capsize=3, label = "w/ Triton on GKE-4gpu, dynamic batching Off")
ax2.errorbar(d4[:,0], d4[:,1], yerr=d4[:,2]/np.sqrt(d4[:,0]), fmt='bv--', linewidth=0.8,markersize=7, markeredgecolor='blue', fillstyle='none', capsize=3, label = "w/ Triton on GKE-4gpu, dynamic batching On")
